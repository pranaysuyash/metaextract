  ...
  try {
    if (isDatabaseConnected()) {
      ...
      const result = await db
        .select({ daily_count: 'daily_count' })
        .from('ip_rate_limits')
        .where('ip', '=', ip)
        .limit(1);
...
async function getIPMinuteCount(ip: string): Promise<number> {
  try {
    if (isDatabaseConnected()) {
      const db = getDatabase();
      const result = await db
        .select({ minute_count: 'minute_count' })
        .from('ip_rate_limits')
        .where('ip', '=', ip)
        .limit(1);
      
      return result[0]?.minute_count || 0;
    }
...
function isDatabaseConnected(): boolean {
  try {
    // This would need to be imported from the actual db module
    // For now, return false to use Redis fallback
    return false;
  } catch {
    return false;
  }
