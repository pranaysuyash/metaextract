...
export class DatabaseStorage implements IStorage {
  ...
  async getCreditGrantByPaymentId(
    ...
    try {
      ...
      console.warn('Credit grants not available:', (error as Error)?.message || error);
      return undefined;
    }
  }
    ...
    try {
      ...
      if (pgError?.code === '25P02') {
        if (process.env.NODE_ENV === 'test') {
          ...
          console.warn('Transaction aborted (25P02) during transferCredits; this usually means a prior statement aborted the transaction. Last operation before abort: ' + (lastOp ?? '<unknown>') + '. Check earlier ERROR logs for the root cause. If this happened during tests, run `npm run db:init-debug` to locate the failing SQL statement.', pgError);
        }
      } else {
    ...
    try {
      ...
      if (pgError?.code === '25P02') {
        if (process.env.NODE_ENV === 'test') {
          ...
          console.warn('Transaction aborted (25P02) during addCredits; this usually means a prior statement aborted the transaction. Last operation before abort: ' + (lastOp ?? '<unknown>') + '. Check earlier ERROR logs for the root cause. If this happened during tests, run `npm run db:init-debug` to locate the failing SQL statement.', pgError);
        }
      } else {
    ...
    try {
      ...
      if (pgError?.code === '25P02') {
        if (process.env.NODE_ENV === 'test') {
          ...
          console.warn('Transaction aborted (25P02) during useCredits; this usually means a prior statement aborted the transaction. Last operation before abort: ' + (lastOp ?? '<unknown>') + '. Check earlier ERROR logs for the root cause. If this happened during tests, run `npm run db:init-debug` to locate the failing SQL statement.', pgError);
        }
      } else {
