...
class EnhancedMetadataExtractor:
    ...
    def extract_metadata(
        ...
        with self.performance_monitor.measure("total_extraction"):
            return self._extract_with_optimizations(
        ...
        try:
            ...
            if tier == "super" and (is_image or is_video):
                try:
                    ...
                    logger.warning(f"AI content detection failed: {e}")
                    advanced_analysis["results"]["ai_content_detection"] = {"error": str(e)}
            
            # Enhanced Steganography analysis (images only)
            if is_image:
                try:
                    logger.info("Performing enhanced steganography analysis...")
                    stego_result = detect_enhanced_steganography(filepath)
                    advanced_analysis["results"]["enhanced_steganography"] = stego_result
                except Exception as e:
                    logger.warning(f"Enhanced steganography analysis failed: {e}")
                    advanced_analysis["results"]["enhanced_steganography"] = {"error": str(e)}
            
            # Enhanced Manipulation detection (images only)
            if is_image:
                try:
                    logger.info("Performing enhanced manipulation detection...")
                    ...
                    logger.warning(f"Enhanced manipulation detection failed: {e}")
                    advanced_analysis["results"]["enhanced_manipulation_detection"] = {"error": str(e)}
            
            # Legacy steganography analysis (for compatibility)
            if STEGANOGRAPHY_AVAILABLE and is_image:
                try:
                    logger.info("Performing legacy steganography analysis...")
                    stego_result = analyze_steganography(filepath)
                    advanced_analysis["results"]["steganography"] = stego_result
                except Exception as e:
                    logger.warning(f"Legacy steganography analysis failed: {e}")
                    advanced_analysis["results"]["steganography"] = {"error": str(e)}
            
            # Legacy manipulation detection (for compatibility)
            if MANIPULATION_AVAILABLE and is_image:
                try:
                    logger.info("Performing legacy manipulation detection...")
                    ...
                    logger.warning(f"Legacy manipulation detection failed: {e}")
                    advanced_analysis["results"]["manipulation_detection"] = {"error": str(e)}
            ...
            if TIMELINE_AVAILABLE:
                try:
                    ...
                    advanced_analysis["results"]["timeline_analysis"] = {"error": str(e)}
    ...
    async def compare_batch_metadata(
        ...
        if tier not in ["premium", "super"]:
            return {"error": "Comparison analysis requires Premium or Super tier"}
    ...
    async def reconstruct_batch_timeline(
        ...
        if not TIMELINE_AVAILABLE:
            return {"error": "Timeline reconstruction module not available"}
        
        if tier not in ["premium", "super"]:
            return {"error": "Timeline reconstruction requires Premium or Super tier"}
        ...
        try:
            ...
            if len(metadata_list) == 0:
                return {"error": "No valid files for timeline reconstruction"}
