...
export interface IStorage {
  ...
  // Credits system
  getOrCreateCreditBalance(sessionId: string, userId?: string): Promise<CreditBalance>;
...
export interface AnalyticsSummary {
  ...
  byMediaType: { images: number; videos: number; pdfs: number; audio: number; other: number };
  last24Hours: number;
  last7Days: number;
...
export class MemStorage implements IStorage {
  ...
  async getAnalyticsSummary(): Promise<AnalyticsSummary> {
    ...
    let images = 0, videos = 0, pdfs = 0, audio = 0, other = 0;
    let last24Hours = 0, last7Days = 0;
    ...
    for (const entry of this.analyticsLog) {
      ...
      byFileType[entry.fileExtension] = (byFileType[entry.fileExtension] || 0) + 1;
    ...
    return {
      ...
      last24Hours,
      last7Days,
      averageProcessingMs: this.analyticsLog.length > 0 ? Math.round(totalProcessingMs / this.analyticsLog.length) : 0,
      totalBytesProcessed: totalBytes,
      successRate: this.analyticsLog.length > 0 ? Math.round((successCount / this.analyticsLog.length) * 100) : 100,
    };
  }

  async getRecentExtractions(limit: number = 50): Promise<ExtractionAnalytics[]> {
    return this.analyticsLog.slice(-limit).reverse();
  }

  async getOrCreateCreditBalance(sessionId: string, userId?: string): Promise<CreditBalance> {
    const existing = Array.from(this.creditBalancesMap.values()).find(b => b.sessionId === sessionId);
  ...
  async getCreditBalance(balanceId: string): Promise<CreditBalance | undefined> {
    return this.creditBalancesMap.get(balanceId);
  }

  async addCredits(balanceId: string, amount: number, description: string, paymentId?: string): Promise<CreditTransaction> {
  ...
  async useCredits(balanceId: string, amount: number, description: string, fileType?: string): Promise<CreditTransaction | null> {
    const balance = this.creditBalancesMap.get(balanceId);
  ...
  async getCreditTransactions(balanceId: string, limit: number = 50): Promise<CreditTransaction[]> {
...
export class DatabaseStorage implements IStorage {
  ...
  async getAnalyticsSummary(): Promise<AnalyticsSummary> {
    ...
    try {
      ...
      return {
        ...
        last24Hours,
        last7Days,
        averageProcessingMs: allEntries.length > 0 ? Math.round(totalProcessingMs / allEntries.length) : 0,
        totalBytesProcessed: totalBytes,
        successRate: allEntries.length > 0 ? Math.round((successCount / allEntries.length) * 100) : 100,
      };
    } catch (error) {
  ...
  async getRecentExtractions(limit: number = 50): Promise<ExtractionAnalytics[]> {
    if (!this.db) return [];
    try {
      return await this.db.select().from(extractionAnalytics).orderBy(desc(extractionAnalytics.requestedAt)).limit(limit);
    } catch (error) {
  ...
  async getOrCreateCreditBalance(sessionId: string, userId?: string): Promise<CreditBalance> {
    ...
    try {
      const existing = await this.db.select().from(creditBalances)
        .where(eq(creditBalances.sessionId, sessionId))
        .limit(1);
      
      if (existing.length > 0) return existing[0];

      const [newBalance] = await this.db.insert(creditBalances)
  ...
  async addCredits(balanceId: string, amount: number, description: string, paymentId?: string): Promise<CreditTransaction> {
    if (!this.db) throw new Error("Database not available");
    try {
      await this.db.update(creditBalances)
        .set({ 
      ...
      const [tx] = await this.db.insert(creditTransactions)
        .values({
          balanceId,
          type: "purchase",
          amount,
...
export const storage: IStorage = isDatabaseConfigured ? new DatabaseStorage() : new MemStorage();

if (!isDatabaseConfigured) {
  console.log("⚠️  DATABASE_URL not configured - using in-memory storage (data will not persist)");
}
