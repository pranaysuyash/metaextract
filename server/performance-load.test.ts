...
describe('Performance and Load Testing', () => {
  ...
  beforeAll(() => {
    ...
    // Setup default storage mocks for all tests
    (storage.hasTrialUsage as jest.Mock).mockResolvedValue(false);
    (storage.getCreditBalanceBySessionId as jest.Mock).mockResolvedValue(null);
    (storage.getOrCreateCreditBalance as jest.Mock).mockResolvedValue({
      id: 'mock-balance-id',
      credits: 10000,
    });
    (storage.useCredits as jest.Mock).mockResolvedValue({ id: 'mock-txn-id' });
    (storage.logExtractionUsage as jest.Mock).mockResolvedValue(undefined);
    (storage.recordTrialUsage as jest.Mock).mockResolvedValue(undefined);
    (storage.saveMetadata as jest.Mock).mockResolvedValue({
      id: 'mock-metadata-id',
      metadataRef: null,
    });
  ...
  describe('Concurrent User Upload Performance', () => {
    ...
    it('should handle 10 concurrent file uploads efficiently', async () => {
      ...
      console.log(
        `10 concurrent uploads completed in ${totalTime.toFixed(2)}ms`
      );
      console.log(
    ...
    it('should maintain performance with 50 concurrent users', async () => {
      ...
      for (let i = 0; i < concurrentRequests; i += batchSize) {
        const batchPromises = Array.from(
          { length: Math.min(batchSize, concurrentRequests - i) },
          (_, j) =>
            request(app)
              .post('/api/extract?tier=enterprise')
              .set('x-test-bypass-credits', '1')
      ...
      console.log(
        `50 concurrent uploads completed in ${totalTime.toFixed(2)}ms`
      );
      console.log(
        `Average time per request: ${(totalTime / concurrentRequests).toFixed(2)}ms`
      );
    ...
    it('should handle sustained load of 100 requests over time', async () => {
      ...
      console.log(
        `100 requests completed in ${totalTime.toFixed(2)}ms (${(totalTime / 1000).toFixed(2)}s)`
      );
      console.log(`Target rate: ${requestsPerSecond} requests/second`);
      console.log(
        `Actual rate: ${(totalRequests / (totalTime / 1000)).toFixed(2)} requests/second`
      );
      console.log(
        `Response times - Avg: ${avgResponseTime.toFixed(2)}ms, Min: ${minResponseTime.toFixed(2)}ms, Max: ${maxResponseTime.toFixed(2)}ms`
      );
  ...
  describe('Batch Processing Performance', () => {
    ...
    it('should process small batches (5 files) efficiently', async () => {
      ...
      console.log(
        `Batch throughput: ${(fileCount / (processingTime / 1000)).toFixed(2)} files/second`
      );
    ...
    it('should scale performance with large batches (50 files)', async () => {
      ...
      console.log(
        `50 files batch would be processed in estimated ${simulatedProcessingTime.toFixed(2)}ms`
      );
      console.log(
        `Estimated average time per file: ${(simulatedProcessingTime / fileCount).toFixed(2)}ms`
      );
      console.log(
        `Estimated batch throughput: ${(fileCount / (simulatedProcessingTime / 1000)).toFixed(2)} files/second`
      );
  ...
  describe('Memory and Resource Usage', () => {
    it('should maintain stable memory usage under load', async () => {
      ...
      memorySnapshots.forEach((snapshot, index) => {
        console.log(
          `  Iteration ${index * 5}: ${(snapshot.heapUsed / 1024 / 1024).toFixed(2)}MB`
        );
      });
      ...
      console.log(
        `Average growth per iteration: ${(memoryGrowth / iterations / 1024).toFixed(2)}KB`
      );
  ...
  describe('API Response Time Performance', () => {
    it('should respond to health checks quickly', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        await request(app).get('/api/extract/health').expect(200);
    ...
    it('should maintain consistent API response times under load', async () => {
      ...
      const stdDev = Math.sqrt(
        responseTimes
          .map(time => Math.pow(time - avgResponseTime, 2))
  ...
  describe('Tier-based Performance', () => {
    it('should process files faster for free tier (fewer fields)', async () => {
      const processingTimesByTier: Record<string, number> = {};
      ...
      for (const tier of ['free', 'professional', 'forensic', 'enterprise']) {
        const mockPythonProcess = {
          stdout: {
            on: jest.fn().mockImplementation((event, callback) => {
              if (event === 'data') {
                const fieldsByTier: Record<string, number> = {
                  free: 50,
                  professional: 500,
                  forensic: 5000,
                  enterprise: 15000,
      ...
      (Object.entries(processingTimesByTier) as Array<[string, number]>).forEach(
        ([tier, time]) => {
      ...
      expect(processingTimesByTier.free).toBeLessThanOrEqual(
        processingTimesByTier.professional + 50
      );
      expect(processingTimesByTier.professional).toBeLessThanOrEqual(
        processingTimesByTier.forensic + 500
      );
    });
  });
});
