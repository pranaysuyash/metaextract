 ...
 */
export function verifyClientToken(token: string): { clientId: string; expiry: number } | null {
  try {
...
export function calculateAbuseScore(
  ...
  const uniqueClientsFromIP = new Set(recentActivity.filter(a => a.ip === ip).map(a => a.clientId)).size;
  if (uniqueClientsFromIP > 3) score += 0.3;
  if (uniqueClientsFromIP > 10) score += 0.2;
  
  // Signal 2: High velocity
  const recentRequests = recentActivity.filter(a => a.timestamp > Date.now() - 60000); // Last minute
 ...
 */
export async function enforceFreeQuota(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
...
async function handleExtractionRequest(
  ...
  try {
    ...
    // Check if in development mode (bypasses all limits)
    if (process.env.NODE_ENV === 'development' && process.env.SKIP_FREE_LIMITS === 'true') {
      next();
      return;
    }
...
export async function handleQuotaExceeded(
  ...
  try {
    ...
    const abuseScore = calculateAbuseScore(clientId, ip, fingerprint, recentActivity);
    ...
    if (abuseScore > CONFIG.ABUSE_SCORE_THRESHOLD) {
      ...
      res.status(429).json({
        ...
        message: 'Free limit reached on this device. Complete verification to continue.',
...
export async function getClientUsage(clientId: string): Promise<{ free_used: number } | null> {
  try {
...
export async function incrementUsage(clientId: string, ip: string): Promise<void> {
  try {
    // Update database
    if (isDatabaseConnected()) {
      try {
...
async function trackRequest(clientId: string, ip: string, userAgent: string, timestamp: number): Promise<void> {
  try {
...
async function getRecentActivity(clientId: string, ip: string, minutes: number = 60): Promise<any[]> {
  try {
    const key = `activity:${clientId}`;
