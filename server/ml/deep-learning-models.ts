...
export class DeepLearningModelManager {
  ...
  constructor() {
    // Avoid heavy native TFJS initialization during tests to prevent
    // native binding/runtime failures in CI/dev environments where
    // @tensorflow/tfjs-node native binaries may not be available.
    if (process.env.NODE_ENV !== 'test') {
      this.initializeModels();
    } else {
      console.log(
        '[DeepLearning] Skipping model initialization in test environment'
      );
    }
  }
  ...
  private async buildRandomForestModel(): Promise<void> {
    ...
    console.log(
      '[DeepLearning] Random Forest baseline model built successfully'
    );
  }
  ...
  public async detectAdvancedThreat(
    ...
    try {
      ...
      console.log(
        `[DeepLearning] Detection complete: threat=${isThreat}, confidence=${confidence}, score=${threatScore}`
      );
  ...
  private async getEnsemblePredictions(
    ...
    try {
      ...
      if (this.models.has('lstm')) {
        ...
        const lstmPrediction = (await lstmModel.predict(
          lstmInput
        )) as tf.Tensor;
      ...
      if (this.models.has('autoencoder')) {
        ...
        const reconstruction = (await autoencoder.predict(
          inputTensor
        )) as tf.Tensor;
   ...
   */
  private async logDeepLearningResult(
    req: Request,
  ...
  private async trainIndividualModel(
      ...
      callbacks: [
        ...
        }),
        {
          onEpochEnd: async (epoch: number, logs: any) => {
            if (epoch % 10 === 0) {
              console.log(
                `[DeepLearning] ${modelName} - Epoch ${epoch}: loss=${logs?.loss?.toFixed(4)}, accuracy=${logs?.accuracy?.toFixed(4)}`
              );
            }
          },
        },
    ...
    const finalAccuracy = Number(
      history.history.accuracy?.[history.history.accuracy.length - 1] || 0
    );
    this.modelPerformance.set(modelName, finalAccuracy);

    console.log(
      `[DeepLearning] ${modelName} final accuracy: ${(finalAccuracy * 100).toFixed(2)}%`
    );
  }
   ...
   * Prepare training data
   */
  private prepareTrainingData(trainingData: any[]): {
    xs: tf.Tensor;
    ys: tf.Tensor;
  } {
