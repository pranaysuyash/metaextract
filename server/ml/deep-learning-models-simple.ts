...
export class SimpleMLModelManager {
  ...
  public async detectAdvancedThreat(
    ...
    try {
      ...
      console.log(`[SimpleML] Detection complete: threat=${isThreat}, confidence=${confidence}, score=${threatScore}`);
   ...
   */
  private getEnsemblePredictions(features: ThreatFeatures): Record<string, number> {
  ...
  private calculateBehavioralScore(features: ThreatFeatures): number {
    ...
    score += features.keystrokeConsistency * weights.KEYSTROKE_CONSISTENCY_WEIGHT;
  ...
  private calculateTemporalScore(features: ThreatFeatures): number {
    ...
    score += (features.requestFrequency / 100) * weights.REQUEST_FREQUENCY_WEIGHT;
    score += (features.sessionDuration / 3600) * weights.SESSION_DURATION_WEIGHT; // Normalize hours
  ...
  private calculateFeatureImportance(features: ThreatFeatures, predictions: Record<string, number>): Record<string, number> {
   ...
   */
  private async logSimpleMLResult(req: Request, result: SimpleMLResult): Promise<void> {
  ...
  public async trainModels(trainingData: any[]): Promise<void> {
    ...
    try {
      ...
      for (const model of Object.keys(ML_CONFIG.ENSEMBLE.WEIGHTS)) {
        ...
        console.log(`[SimpleML] ${model} model accuracy: ${(newPerf * 100).toFixed(2)}%`);
      }
  ...
  public async updateModelOnline(
    ...
    console.log(`[SimpleML] Online learning update: label=${label}, learningRate=${learningRate}`);
    
    // In a real implementation, this would adjust model weights
    // For now, we just track that learning occurred
    this.modelPerformance.set('online_learning', (this.modelPerformance.get('online_learning') || 0.9) + 0.001);
  }
}
