...
# Add extractor package to Python path for proper imports
# Extract the repository root and add both server and extractor to path
repo_root = Path(__file__).parent
extractor_root = repo_root / 'server' / 'extractor'

# Add the extractor package itself to path (makes 'extractor.modules.' work)
sys.path.insert(0, str(extractor_root))

# Import from modules subpackage using absolute imports with full package name
# The 'extractor' must be available as a package in sys.path
try:
...
from modules.iptc_xmp import get_iptc_field_count
from modules.images import get_image_field_count
from modules.geocoding import get_geocoding_field_count
from modules.colors import get_color_field_count
from modules.quality import get_quality_field_count
from modules.time_based import get_time_based_field_count
from modules.video import get_video_field_count
from modules.audio import get_audio_field_count
from modules.svg import get_svg_field_count
from modules.psd import get_psd_field_count
from modules.perceptual_hashes import get_perceptual_hash_field_count
from modules.iptc_xmp_fallback import get_fallback_field_count
from modules.video_keyframes import get_keyframe_field_count
from modules.directory_analysis import get_directory_field_count
from modules.mobile_metadata import get_mobile_field_count
from modules.quality_metrics import get_quality_field_count as get_quality_metrics_field_count
from modules.drone_metadata import get_drone_field_count
from modules.icc_profile import get_icc_field_count
from modules.camera_360 import get_360_field_count
from modules.accessibility_metadata import get_accessibility_field_count
from modules.vendor_makernotes import get_makernote_field_count
from modules.makernotes_complete import get_makernote_field_count as get_complete_makernote_field_count, get_vendor_field_count
from modules.social_media_metadata import get_social_media_field_count
from modules.forensic_metadata import get_forensic_metadata_field_count
from modules.web_metadata import get_web_metadata_field_count
from modules.action_camera import get_action_camera_field_count
from modules.scientific_medical import get_scientific_field_count
from modules.print_publishing import get_print_publishing_field_count
from modules.workflow_dam import get_workflow_dam_field_count
...
try:
    ...
    AUDIO_METADATA_EXTENDED_AVAILABLE = True
except:
    AUDIO_METADATA_EXTENDED_AVAILABLE = False
...
if AUDIO_METADATA_EXTENDED_AVAILABLE:
    audio_extended_count = get_audio_extended_field_count()
    print(f'{"Audio Metadata Extended":30s}: {audio_extended_count:>5} fields')
    total += audio_extended_count
    phase2_total += audio_extended_count
else:
    print(f'{"Audio Metadata Extended":30s}: {0:>5} fields (pending)')
...
fields3['Forensic/Security'] = get_forensic_metadata_field_count()
...
# Additional master consolidation files - NEW
try:
    from image_master import get_image_master_field_count
    IMAGE_MASTER_AVAILABLE = True
except:
    IMAGE_MASTER_AVAILABLE = False

try:
    from maker_master import get_maker_master_field_count
    MAKER_MASTER_AVAILABLE = True
except:
    MAKER_MASTER_AVAILABLE = False

print()
print('--- Additional Master Consolidation Files (NEW) ---')
additional_master_total = 0

if IMAGE_MASTER_AVAILABLE:
    image_master_count = get_image_master_field_count()
    print(f'{"Image Master":30s}: {image_master_count:>5} fields')
    total += image_master_count
    additional_master_total += image_master_count
else:
    print(f'{"Image Master":30s}: {0:>5} fields (pending)')

if MAKER_MASTER_AVAILABLE:
    maker_master_count = get_maker_master_field_count()
    print(f'{"Maker Master":30s}: {maker_master_count:>5} fields')
    total += maker_master_count
    additional_master_total += maker_master_count
else:
    print(f'{"Maker Master":30s}: {0:>5} fields (pending)')

print(f'{"Additional Masters Total":30s}: {additional_master_total:>5} fields')
