 ...
 *
 * REVISION: Added rate limiting bypass handling
 */
...
const TEST_USER = {
  email: 'security-test2@example.com', // Different email to avoid rate limiting
...
async function testLoginSecurity() {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      // Rate limited from previous brute force test
      addResult(
...
async function testPasswordResetSecurity() {
  ...
  // Test 3: Empty/short reset token (NEW TEST)
  try {
    const res = await makeRequest('POST', '/api/auth/password-reset/confirm', {
      token: '',
      password: 'NewSecurePass456!',
    ...
    addResult(
      ...
      rejected,
      rejected ? 'Empty token properly rejected' : `Status: ${res.status}`,
      'high'
    );
  } catch (e) {
    addResult('Empty reset token rejected', false, e.message, 'high');
  }
}
...
async function testSessionSecurity() {
  ...
  // Test 4: Cookie cleared after logout (NEW TEST)
  try {
    ...
    const cleared =
      cookieHeaders &&
      cookieHeaders.some(
        c => c.includes('auth_token=;') || c.includes('Max-Age=0')
      );
    addResult(
      'Cookie cleared on logout',
      cleared,
      cleared ? 'Auth cookie cleared' : 'Cookie not properly cleared',
      'medium'
    );
  } catch (e) {
    addResult('Cookie cleared on logout', false, e.message, 'medium');
  }
}
