...
import { advancedProtectionMiddleware, verifyChallengeResponse, getProtectionStats } from '../server/middleware/advanced-protection';
import { generateFingerprint, trackFingerprint } from '../server/monitoring/browser-fingerprint';
import { mlAnomalyDetector } from '../server/monitoring/ml-anomaly-detection';
import { securityEventLogger } from '../server/monitoring/security-events';

// Mock dependencies
jest.mock('../server/monitoring/browser-fingerprint');
jest.mock('../server/monitoring/ml-anomaly-detection');
jest.mock('../server/monitoring/security-events');
jest.mock('../server/monitoring/security-alerts');
...
describe('Advanced Protection System', () => {
  ...
  describe('Browser Fingerprinting', () => {
    it('should generate fingerprint from request data', async () => {
      ...
      expect(generateFingerprint).toHaveBeenCalledWith(mockReq, undefined);
    ...
    it('should detect anomalies in fingerprint data', async () => {
      ...
      (generateFingerprint as jest.Mock).mockResolvedValue(suspiciousFingerprint);
  ...
  describe('ML Anomaly Detection', () => {
    it('should detect normal behavior', async () => {
      ...
      (mlAnomalyDetector.detectUploadAnomaly as jest.Mock).mockResolvedValue(mockAnomalyResult);

      const result = await mlAnomalyDetector.detectUploadAnomaly(mockReq as Request, mockFingerprint);
    ...
    it('should detect anomalous behavior', async () => {
      ...
      const mockAnomalyResult = {
        ...
        riskScore: 75,
        riskLevel: 'high',
        contributingFactors: ['High upload frequency', 'Headless browser detected'],
        recommendations: ['Implement additional verification', 'Consider rate limiting'],
        modelVersion: '1.0.0',
        timestamp: new Date()
      };
      ...
      const result = await mlAnomalyDetector.detectUploadAnomaly(mockReq as Request, suspiciousFingerprint);
  ...
  describe('Advanced Protection Middleware', () => {
    ...
    it('should block high-risk requests', async () => {
      ...
      const highRiskAnomaly = {
        ...
        riskScore: 85,
        riskLevel: 'high',
        contributingFactors: ['High upload frequency', 'Suspicious fingerprint'],
        recommendations: ['Block request', 'Monitor device'],
        modelVersion: '1.0.0',
        timestamp: new Date()
      };

      (mlAnomalyDetector.detectUploadAnomaly as jest.Mock).mockResolvedValue(highRiskAnomaly);
    ...
    it('should challenge medium-risk requests', async () => {
      ...
      (mlAnomalyDetector.detectUploadAnomaly as jest.Mock).mockResolvedValue(mediumRiskAnomaly);
    ...
    it('should handle errors gracefully', async () => {
      ...
      (generateFingerprint as jest.Mock).mockRejectedValue(new Error('Fingerprint error'));
  ...
  describe('Security and Privacy', () => {
    ...
    it('should handle privacy-sensitive data appropriately', async () => {
      ...
      const loggedEvents = (securityEventLogger.logEvent as jest.Mock).mock.calls;
      const hasUserAgentInDetails = loggedEvents.some(call => 
        call[0].details && call[0].details.userAgent
      );
