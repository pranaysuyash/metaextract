...
export const TIER_CONFIGS: Record<TierName, TierConfig> = {
  ...
  free: {
    ...
    fieldCount: "200-300",
    fieldCategories: ["summary", "basic_exif", "image", "gps", "hashes", "calculated"],
    description: "Basic EXIF + GPS + file hashes. Perfect for evaluation before upgrading.",
  },
  ...
  professional: {
    ...
    allowedFileTypes: [
      ...
      // RAW formats
      "image/x-canon-cr2", "image/x-canon-cr3", "image/x-nikon-nef", "image/x-nikon-nrw",
      "image/x-sony-arw", "image/x-sony-sr2", "image/x-adobe-dng",
      "image/x-olympus-orf", "image/x-panasonic-rw2", "image/x-panasonic-raw",
      "image/x-fuji-raf", "image/x-pentax-pef", "image/x-leica-rwl",
      "image/x-hasselblad-3fr", "image/x-phaseone-iiq",
      // HEIC
    ...
    fieldCount: "2,000+",
    fieldCategories: [
      "summary", "exif", "image", "gps", "filesystem", "hashes", 
      "calculated", "forensic", "makernote", "iptc", "xmp", "extended_attributes"
    ],
    description: "Full EXIF + MakerNotes + IPTC/XMP. All image formats including RAW.",
  },
  ...
  forensic: {
    ...
    fieldCount: "7,000+",
    fieldCategories: ["*"],
    description: "Complete 7,000+ field extraction. Video, audio, batch processing. API access.",
  },
  ...
  enterprise: {
    ...
    fieldCount: "7,000+",
    fieldCategories: ["*"],
    description: "Everything in Forensic + unlimited API, 2GB files, dedicated support, compliance reporting.",
  },
};
...
export function normalizeTier(tier: string): TierName {
  ...
  return LEGACY_TIER_MAP[normalized] || (TIER_CONFIGS[normalized as TierName] ? normalized as TierName : "enterprise");
}
...
export function getRequiredTierForFileType(mimeType: string): TierName {
  ...
  const rawIndicators = ["raw", "cr2", "cr3", "nef", "arw", "dng", "orf", "rw2", "raf", "pef", "rwl", "3fr", "iiq"];
...
export function getRateLimits(tier: string): { requestsPerMinute: number; requestsPerDay: number } {
  const normalizedTier = normalizeTier(tier);
