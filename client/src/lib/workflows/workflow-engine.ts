...
export interface WorkflowTrigger {
  id: string;
  type: 'file_uploaded' | 'extraction_completed' | 'schedule' | 'manual' | 'webhook';
...
export interface WorkflowAction {
  id: string;
  type: 'send_email' | 'export_data' | 'call_webhook' | 'update_database' | 'notify_user';
  config: Record<string, any>;
...
export class WorkflowAutomation {
    ...
    this.templates = [
      {
        ...
        description: 'Send an email notification when metadata extraction is completed',
      ...
      },
      {
        ...
        description: 'Call a webhook when GPS coordinates are found in metadata',
      ...
      },
      {
        ...
        description: 'Automatically export metadata for all files in a folder on a schedule',
        category: 'automation',
        isBuiltIn: true,
        triggers: [
          {
   ...
   */
  async deactivateWorkflow(workflowId: string, userId: string): Promise<boolean> {
    const workflow = await this.getWorkflow(workflowId, userId);
    if (!workflow) return false;
  ...
  private isTriggerMatch(trigger: WorkflowTrigger, event: any): boolean {
    switch (trigger.type) {
      ...
      case 'webhook':
        return event.type === 'webhook' && event.source === trigger.config.source;
      case 'schedule':
   ...
   * Execute workflow actions
   */
  private async executeActions(workflow: Workflow, triggerEvent: any): Promise<any> {
    ...
    for (const action of workflow.actions) {
      ...
      try {
        ...
        context[action.id] = result; // Store result in context for next action
      } catch (error) {
   ...
   */
  private createVariableContext(variables: WorkflowVariable[]): Record<string, any> {
  ...
  async getWorkflowStats(workflowId: string, userId: string): Promise<{
    ...
    const lastExecution = executions.length > 0 ? executions[0].startedAt : null;
  ...
  async validateWorkflow(workflow: Workflow): Promise<{ isValid: boolean; errors: string[] }> {
    ...
    for (const condition of workflow.conditions) {
      if (condition.nextActionId && !workflow.actions.some(a => a.id === condition.nextActionId)) {
        errors.push(`Condition ${condition.id} references non-existent action: ${condition.nextActionId}`);
      }
      if (condition.elseActionId && !workflow.actions.some(a => a.id === condition.elseActionId)) {
        errors.push(`Condition ${condition.id} references non-existent else action: ${condition.elseActionId}`);
      }
    }

    // Validate action sequence references
    for (const action of workflow.actions) {
      if (action.nextActionId && !workflow.actions.some(a => a.id === action.nextActionId)) {
        errors.push(`Action ${action.id} references non-existent next action: ${action.nextActionId}`);
      }
    }
