...
const getFileTypeColor = (type: string) => {
  if (type.startsWith('image/')) return 'bg-blue-500/10 text-blue-600 border-blue-200';
  if (type.startsWith('video/')) return 'bg-purple-500/10 text-purple-600 border-purple-200';
  if (type.startsWith('audio/')) return 'bg-green-500/10 text-green-600 border-green-200';
  if (type === 'application/pdf') return 'bg-red-500/10 text-red-600 border-red-200';
...
const getTierColor = (tier: string) => {
  switch (tier) {
    ...
    case 'forensic': return 'bg-purple-100 text-purple-800';
...
const getHighlightIcon = (highlight: string) => {
  if (highlight.toLowerCase().includes('gps') || highlight.toLowerCase().includes('location')) return MapPin;
  if (highlight.toLowerCase().includes('camera') || highlight.toLowerCase().includes('lens')) return Camera;
  if (highlight.toLowerCase().includes('codec') || highlight.toLowerCase().includes('stream')) return Video;
  return Zap;
};

export function SampleFiles({ onSampleSelect, currentTier, className }: SampleFilesProps) {
  ...
  const handleSampleSelect = async (sample: SampleFile) => {
    ...
    try {
      ...
      const downloadResponse = await fetch(`/api/samples/${sample.id}/download`);
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to process sample file",
        variant: "destructive"
      });
    } finally {
  ...
  const canAccessSample = (sampleTier: string) => {
    ...
    return tierLevels[currentTier as keyof typeof tierLevels] >= tierLevels[sampleTier as keyof typeof tierLevels];
  };
  ...
  if (loading) {
    return (
      <div className={cn("flex items-center justify-center p-8", className)}>
        ...
        <span className="ml-2 text-muted-foreground">Loading sample files...</span>
      </div>
    );
  }
              ...
              <Card className={cn(
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge 
                      variant="outline" 
                      className={cn("text-xs", getTierColor(sample.tier_required))}
                    >
