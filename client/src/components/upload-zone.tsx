...
export function UploadZone() {
  ...
  useEffect(() => {
    ...
    const stored = window.localStorage.getItem('metaextract_access_email');
  ...
  const startUpload = async (file: File) => {
    ...
    try {
      ...
      if (accessEmail) {
        formData.append('access_email', accessEmail);
      }
  ...
  const handleAccessConfirm = (email: string) => {
    setAccessEmail(email);
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('metaextract_access_email', email);
    }
