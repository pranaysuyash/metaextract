...
const FILE_CONFIG = {
  ...
  supportedCategories: {
    images: [
      ...
      'image/heic', 'image/heif', 'image/bmp', 'image/svg+xml', 'image/x-icon',
    ...
    specialized: [
      'application/dicom', 'application/fits', 'application/x-hdf5',
  ...
  supportedExtensions: [
    ...
    '.bmp', '.svg', '.ico', '.cr2', '.cr3', '.nef', '.arw', '.dng', '.raf',
    '.orf', '.rw2', '.pef', '.raw', '.mp4', '.mov', '.avi', '.mkv', '.webm',
    '.mpeg', '.mpg', '.3gp', '.flv', '.mp3', '.wav', '.flac', '.aac', '.ogg',
    '.m4a', '.pdf', '.doc', '.docx', '.dcm', '.dicom', '.fits', '.fit', '.fts',
    '.h5', '.hdf5', '.nc', '.nc4'
  ]
};
...
function validateFile(file: File): ValidationResult {
  ...
  if (file.size > FILE_CONFIG.maxSizeBytes) {
    return {
      valid: false,
      error: `File size (${(file.size / 1024 / 1024).toFixed(1)}MB) exceeds maximum of ${FILE_CONFIG.maxSizeMB}MB`
    };
  }
  ...
  // Check MIME type
  const allSupportedMimes = Object.values(FILE_CONFIG.supportedCategories).flat();
  const mimeSupported = allSupportedMimes.includes(file.type);

  // Check extension as fallback
  const extension = '.' + file.name.split('.').pop()?.toLowerCase();
  const extensionSupported = FILE_CONFIG.supportedExtensions.includes(extension);

  if (!mimeSupported && !extensionSupported) {
    return {
      valid: false,
      error: `Unsupported file type: ${file.type || extension}. We support images, videos, audio, PDFs, and specialized formats (DICOM, FITS, etc.)`
    };
  }
    ...
    if (files && files.length > 0) {
      if (files.length > 1) {
        toast({
          ...
          description: "Only the first file will be processed. Use batch upload for multiple files.",
          variant: "default",
        });
      }
      processFile(files[0]);
            ...
            {currentFile && (
              <div className="mb-6 p-3 bg-white/5 rounded-lg border border-white/10">
                <div className="text-xs text-slate-400 font-mono">
                  <span className="text-slate-500">FILE:</span> {currentFile.name}
                </div>
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
              ...
              <div className="text-slate-500 text-xs mb-8 space-y-1">
                ...
                <p className="text-slate-600">Max file size: {FILE_CONFIG.maxSizeMB}MB</p>
              </div>
          ...
          >
            ...
            <div className="space-y-2 relative z-10">
              ...
              <div className="flex justify-between text-[10px] font-mono text-slate-500 uppercase">
                <span className={uploadProgress > 10 ? "text-primary" : ""}>Header</span>
                <span className={uploadProgress > 40 ? "text-primary" : ""}>Body</span>
                <span className={uploadProgress > 70 ? "text-primary" : ""}>Analysis</span>
                <span className={uploadProgress > 95 ? "text-primary" : ""}>Complete</span>
              </div>
            </div>
            ...
            <div className="mt-8 grid grid-cols-3 gap-2 relative z-10">
              <div className="bg-white/5 p-2 rounded border border-white/5 text-center">
                ...
                <span className="text-[10px] text-slate-400 block">45K+ Fields</span>
              </div>
              <div className="bg-white/5 p-2 rounded border border-white/5 text-center">
