...
export function SimpleUploadZone() {
    ...
    const handleFile = (file: File) => {
        ...
        const isSupportedMime = file.type === 'image/jpeg' || file.type === 'image/png';
    ...
    const uploadFile = async (file: File, email: string) => {
        ...
        try {
            ...
            if (!res.ok) {
                if (res.status === 402) {
                    ...
                    toast({
                        ...
                        description: "You've used your 2 free checks. Unlock more with credits.",
                        variant: "destructive"
                    });
                    setShowPricingModal(true);
                    return;
                }
                const message = data?.error || data?.message || responseText || 'Extraction failed';
            ...
            console.error(err);
            const fallbackMessage = typeof err?.message === 'string' ? err.message : 'Network error';
            if (fallbackMessage.toLowerCase().includes('failed to fetch')) {
                toast({
                    title: "Backend unavailable",
                    description: "API is not reachable. Start the server with `npm run dev:server`.",
                    variant: "destructive"
                });
                return;
            }
            ...
            >
                ...
                {isUploading ? (
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-white font-mono text-sm animate-pulse">Extracting Metadata...</p>
                    </div>
                ) : (
                    <>
                        <div className="mb-4">
