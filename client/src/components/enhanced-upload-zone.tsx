...
const ACCEPTED_TYPES = {
  'image/*': ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.tiff', '.bmp', '.heic', '.heif', '.svg'],
...
const getFileTypeColor = (file: File) => {
  if (file.type.startsWith('image/')) return 'bg-blue-500/10 text-blue-600 border-blue-200';
  ...
  if (file.type === 'application/pdf') return 'bg-red-500/10 text-red-600 border-red-200';
    ...
    try {
      ...
      if (files.length === 1) {
        ...
        setFiles(prev => prev.map(file => {
          const result = batchResult.results[file.name];
          return {
            ...file,
