...
interface BatchGridProps {
  ...
  sortOrder?: 'asc' | 'desc';
  onSortChange?: (
    field: 'filename' | 'date' | 'fields' | 'size' | 'status',
    order: 'asc' | 'desc'
  ) => void;
  onFileAction?: (
    action: 'download' | 'view' | 'reprocess',
    fileId: string
  ) => void;
}
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem
                            onClick={() => onFileAction?.('view', result.id)}
                          >
                          ...
                          {result.status === 'success' && (
                            <DropdownMenuItem
                              onClick={() =>
                                onFileAction?.('download', result.id)
                              }
                            >
                          ...
                          {result.status === 'error' && (
                            <DropdownMenuItem
                              onClick={() =>
                                onFileAction?.('reprocess', result.id)
                              }
                            >
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-12 gap-4 p-3 bg-muted/30 rounded-lg text-sm font-medium text-slate-300">
        ...
        <button
          ...
          className="col-span-4 flex items-center gap-2 cursor-pointer hover:text-white text-left"
        ...
        <button
          ...
          className="col-span-1 flex items-center gap-2 cursor-pointer hover:text-white text-left"
        ...
        <button
          ...
          className="col-span-2 flex items-center gap-2 cursor-pointer hover:text-white text-left"
          ...
          <div className="col-span-1 flex items-center">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {result.status === 'success' && (
                  <DropdownMenuItem
                    onClick={() => onFileAction?.('download', result.id)}
                  >
                    <Download className="w-4 h-4 mr-2" />
                    Download Metadata
                  </DropdownMenuItem>
                ...
                {result.status === 'error' && (
                  <DropdownMenuItem
                    onClick={() => onFileAction?.('reprocess', result.id)}
                  >
                    <RefreshCw className="w-4 h-4 mr-2" />
                    Reprocess File
                  </DropdownMenuItem>
