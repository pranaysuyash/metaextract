...
export default function ImagesMvpResults() {
    ...
    const isPurposeValue = (value: string): value is PurposeValue =>
        value === "privacy" || value === "authenticity" || value === "photography" || value === "explore";
    ...
    // Format Date Helper
    const formatDate = (dateStr?: string, emptyMessage = "Not present in this file") => {
    ...
    const highlightAccent = (intent: "Privacy" | "Authenticity" | "Photography") => {
        if (intent === "Privacy") return "border-emerald-500/20 bg-emerald-500/5 text-emerald-200";
        if (intent === "Authenticity") return "border-purple-500/20 bg-purple-500/5 text-purple-200";
    ...
    const buildSummaryLines = () => {
        ...
        orderedHighlights.slice(0, 6).forEach((h) => {
            lines.push(`- ${h.text} (Impact: ${h.impact}, Confidence: ${h.confidence})`);
        });
        lines.push("");
        lines.push("Limitations: Metadata can be missing or stripped. Absence is not proof.");
        return lines.join("\n");
    };
    ...
    const purposeLabel = purpose
        ? `${purpose.charAt(0).toUpperCase()}${purpose.slice(1)}`
        : "Not set";
    const trialEmail = typeof window !== "undefined"
    ...
    const registrySummary = metadata.registry_summary?.image as
        | { exif?: number; iptc?: number; xmp?: number; mobile?: number; perceptual_hashes?: number }
        | undefined;
    const lockedGroups = [
        { key: "exif", label: "Full EXIF fields", count: registrySummary?.exif ?? 0 },
        { key: "iptc", label: "IPTC (author/rights)", count: registrySummary?.iptc ?? 0 },
        { key: "xmp", label: "XMP (edit history)", count: registrySummary?.xmp ?? 0 },
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p className="text-slate-300 text-xs leading-relaxed">
                                    ...
                                    Unlock credits to view the full report and raw exports.
                                </p>
                            </div>
                    ...
                    {formatHint && (
                        <Card className={`mb-6 border ${formatToneClass}`}>
                            <CardContent className="pt-6 flex items-start gap-3 text-sm">
                                ...
                                <div>
                                    <div className="font-semibold text-white">{formatHint.title}</div>
                                    <div className="text-xs text-slate-300 mt-1">{formatHint.body}</div>
                                </div>
                            </CardContent>
                        </Card>
                    )}
                                    ...
                                    <CardContent>
                                        {hasGps && gpsCoords ? (
                                            ...
                                            <div className="py-8 text-center">
                                                ...
                                                <p className="text-emerald-500 font-bold">Location not present</p>
                                                <p className="text-slate-500 text-xs mt-1">No GPS coordinates were found in this file.</p>
                                            </div>
                                        )}
                                    </CardContent>
                                </Card>
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="grid grid-cols-1 gap-4">
                                            {!hasValue(metadata.exif?.Make) && !hasValue(metadata.exif?.Model) ? (
                                                <div className="text-xs text-slate-500">No camera make/model tags present in this file.</div>
                                            ) : (
                                                <>
                                        ...
                                        <div className="grid grid-cols-1 gap-4">
                                            <div className="pb-3 border-b border-white/5">
                                                ...
                                                <span className="text-white font-medium">
                                                    {captureDateValue ? formatDate(captureDateValue) : "Not present in this file"}
                                                </span>
                                            </div>
                                    ...
                                    <CardContent>
                                        {(() => {
                                            ...
                                            if (!hasMakerNotes && !hasValue(serial) && !hasValue(colorProfile)) {
                                                return <div className="text-xs text-slate-500">No hidden identifiers present in this file.</div>;
                                            }
                                                                ...
                                                                <>
                                                                    {!canExport && (
                                                                        <div className="text-xs text-slate-500 mb-3">
                                                                            Showing the first {maxEntries} entries. Unlock the full report to search and view everything.
                                                                        </div>
                                                                    )}
                                ...
                                <Card className="bg-[#121217] border-white/5" id="section-auth-signals">
                                    ...
                                    <CardContent className="space-y-3 text-sm">
                                        <div className="text-slate-300">
                                            ...
                                            <span className="text-white">{hasValue(software) ? software : "No tag present (inconclusive)"}</span>
                                        </div>
                                        <div className="text-slate-300">
                                            <span className="text-slate-500 block text-xs font-mono mb-1">METADATA STATE</span>
                                            <span className="text-white">{metadata.metadata_comparison?.summary?.overall_status || "Not available for this file"}</span>
                                        </div>
                                        {metadata.metadata_comparison?.warnings?.length ? (
                                            ...
                                            <div className="text-xs text-slate-500">No additional authenticity warnings present.</div>
                                        )}
                                    </CardContent>
                                </Card>
                                ...
                                <Card className="bg-[#121217] border-white/5">
                                    ...
                                    <CardContent className="space-y-3 text-sm font-mono">
                                        ...
                                        {!hasValue(metadata.perceptual_hashes?.phash) && !hasValue(metadata.perceptual_hashes?.dhash) && (
                                            <div className="text-xs text-slate-500">Perceptual hashes not present for this file.</div>
                                        )}
                                    </CardContent>
                                </Card>
                                ...
                                <Card className="bg-[#121217] border-white/5">
                                    ...
                                    <CardContent className="space-y-3 text-sm">
                                        ...
                                        {!hasValue(metadata.exif?.ISO) && !hasValue(metadata.exif?.FNumber) && !hasValue(metadata.exif?.ExposureTime) && !hasValue(metadata.exif?.FocalLength) && (
                                            <div className="text-xs text-slate-500">No camera setting fields present in this file.</div>
                                        )}
                                    </CardContent>
                                </Card>
                                ...
                                <Card className="bg-[#121217] border-white/5">
                                    ...
                                    <CardContent className="space-y-3 text-sm">
                                        {!dimensionsValue && !megapixelsValue && !colorSpaceValue ? (
                                            <div className="text-xs text-slate-500">No image info fields present in this file.</div>
                                        ) : (
                                            <>
                                                {dimensionsValue && (
                                                    <div className="text-slate-300">
                                ...
                                <Card className="bg-[#121217] border-white/5 md:col-span-2">
                                    ...
                                    <CardContent className="space-y-3">
                                        <div className="text-xs text-slate-500">
                                            ...
                                            {!canExport && exifEntries.length > 14 ? " • Unlock the full report to see all fields" : ""}
                                        </div>
                                        {exifEntries.length === 0 ? (
                                            <div className="text-xs text-slate-500">No EXIF fields present in this file.</div>
                                        ) : (
                                    ...
                                    <CardContent>
                                        {!canExport && (
                                            <div className="text-sm text-slate-400 mb-4">
                                                Raw JSON export is locked during the trial, but you can preview and search a subset of extracted fields here.
                                            </div>
                                        )}
