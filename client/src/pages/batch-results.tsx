...
export const BatchResultsPage: React.FC = () => {
  ...
  const { data: batchResults = [], isLoading: isLoadingResults } = useQuery<BatchResult[]>({
    queryKey: [`/api/batch/jobs/${currentBatchJob?.id}/results`],
    queryFn: async () => {
      if (!currentBatchJob?.id) return [];
      const response = await apiRequest('GET', `/api/batch/jobs/${currentBatchJob.id}/results`);
      return response.json();
    },
  ...
  const summary = React.useMemo((): BatchSummary => {
    ...
    const processingFiles = filteredResults.filter(r => r.status === 'processing').length;
    const pendingFiles = filteredResults.filter(r => r.status === 'pending').length;
    const totalFields = filteredResults.reduce((sum, r) => sum + r.fieldsExtracted, 0);
      ...
      <div className="container mx-auto py-8 px-4">
        <Card className="bg-card border-white/10">
          <CardContent className="py-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-white mb-2">Professional Feature</h2>
      ...
      <div className="mb-8">
        ...
        {currentBatchJob && (
          <div className="flex items-center gap-4 text-sm text-slate-300">
            ...
            <span>•</span>
            <span>Created: {new Date(currentBatchJob.createdAt).toLocaleString()}</span>
            <span>•</span>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{summary.totalFiles}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-emerald-400">{summary.successfulFiles}</p>
                  <p className="text-xs text-slate-400">{Math.round(summary.successRate)}%</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-400">{summary.errorFiles}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-400">{summary.processingFiles}</p>
                </div>
      ...
      {currentBatchJob && (
        <div className="flex flex-wrap gap-3 mb-6">
          <Button 
            ...
            className="gap-2"
          >
            {selectedFiles.length === paginatedResults.length ? 'Deselect All' : 'Select All'}
          </Button>
          ...
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {paginatedResults.map((result) => (
                <Card key={result.id} className="bg-card border-white/10 hover:border-white/20 transition-colors">
                  <CardContent className="p-4">
                    ...
                    <h3 className="font-medium text-white mb-2 truncate" title={result.filename}>
                      {result.filename}
                    </h3>
                    ...
                    <div className="space-y-1 text-sm text-slate-300">
                      ...
                      <div className="flex justify-between">
                        <span>Size:</span>
                        <span className="text-white">{Math.round(result.fileSize / 1024)}KB</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Type:</span>
                        <span className="text-white text-xs">{result.fileType}</span>
                      </div>
                    </div>
                  ...
                  <thead className="border-b border-white/10">
                    <tr>
                      <th className="text-left p-4">
                        <input
                          type="checkbox"
                          checked={selectedFiles.length === paginatedResults.length && paginatedResults.length > 0}
                      ...
                      <th className="text-left p-4 text-slate-300 font-medium">Fields</th>
                  ...
                  <tbody>
                    {paginatedResults.map((result) => (
                      <tr key={result.id} className="border-b border-white/5 hover:bg-white/5">
                        ...
                        <td className="p-4 text-white">{result.fieldsExtracted}</td>
                        <td className="p-4 text-white">{Math.round(result.fileSize / 1024)}KB</td>
                        <td className="p-4 text-white text-sm">{result.fileType}</td>
