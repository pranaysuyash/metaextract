...
4. **DATABASE_LAUNCH_VERIFICATION.md** - Launch-critical database checklist (NEW)
...
### Issue #4: Database Connection Pool Too Small (NEW)
**Severity**: 🔴 **BLOCKING**  
**Problem**: Configured for 10 connections, fails at 11  
**Impact**: 503 errors at scale (launch failure)  
**Fix Time**: 1 hour (config change + test)  
**Status**: NEEDS FIX

### Issue #5: Critical Performance Indexes Missing (NEW)
**Severity**: 🔴 **BLOCKING**  
**Problem**: High-traffic tables missing indexes  
**Impact**: Queries degrade rapidly with load  
**Fix Time**: 1-2 hours (SQL script)  
...
### Issue #6: No User Data Isolation at Scale
**Severity**: 🟠 **HIGH**  
...
6. **Database Pool** - Test with 20+ concurrent connections
7. **Database Indexes** - Verify all indexes exist via `\d` commands
8. **Payment** - End-to-end payment flow on desktop and mobile
