...
ISSUE #4: DB Connection Pool (NEW)
Problem: Max 10 connections = fails at 11 users
Fix: Increase to 20+ in server/db.ts
Impact: 503 errors at launch
Fix Time: 1 hour

ISSUE #5: Performance Indexes (NEW)
Problem: Missing indexes on analytics tables
Fix: Add missing indexes (see DB doc)
