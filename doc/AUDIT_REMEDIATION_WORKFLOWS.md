<!-- markdownlint-disable MD029 MD033 MD037 MD034 -->
...
- keep extension
  Example:
  server/auth.ts -> docs/audit/server\_\_auth.ts.md
...
- rg -n --hidden --no-ignore -S "<file-basename>|<exported symbol>|<route path>|<function name>" test tests **tests** .
...
- git diff --name-only <base>..<head> | rg -n "(test|spec|**tests**|tests/)"
...
REQUIRED OUTPUT STRUCTURE 0) Evidence Appendix
...
- Blast radius (HIGH/MED/LOW)
  Compute a Priority Score (1–27) = Severity(1–3) _ Likelihood(1–3) _ Blast(1–3).
  Tie-breakers:
...
- [ ] (score=\_\_) <Title> — target: <file> — anchor: <anchor> — basis: <basis> — dedupe_key: <...>
